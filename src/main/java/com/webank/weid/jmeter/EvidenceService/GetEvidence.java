...
public class GetEvidence extends AbstractJavaSamplerClient {
    ...
    public static void main(String[] args) {
        ...
        GetEvidence test = new GetEvidence();
        test.setupTest(arg0);
        test.runTest(arg0);
    ...
    public void setupTest(JavaSamplerContext context) {
        getNewLogger().info("--------------------->this is GetEvidence setupTest");
...
//        arguments.addArgument("name", this.name);
    ...
    public SampleResult runTest(JavaSamplerContext context) {
        ...
        result.setSampleLabel("GetEvidence");
        result.sampleStart();
        ResponseData<EvidenceInfo> rr = evidenceService.getEvidence(hash);
        if (rr.getResult() == null) {
            try {
                throw new Exception("GetEvidence:" +
                        rr.getErrorCode() + ", " +
                        rr.getErrorMessage());
            } catch (Exception e) {
                e.printStackTrace();
                getNewLogger().error("------------->GetEvidence fail:", e);
                result.sampleEnd();
            ...
            getNewLogger().info("----------------->GetEvidence"+rr.toString());
            result.sampleEnd();
        }
        return result;
    }
