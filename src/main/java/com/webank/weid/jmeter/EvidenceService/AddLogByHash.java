...
public class AddLogByHash extends AbstractJavaSamplerClient {
    ...
    public static void main(String[] args) {
        ...
        AddLogByHash test = new AddLogByHash();
        test.setupTest(arg0);
        test.runTest(arg0);
    ...
    public void setupTest(JavaSamplerContext context) {
        getNewLogger().info("--------------------->this is AddLogByHash setupTest");
...
//        arguments.addArgument("name", this.name);
    ...
    public SampleResult runTest(JavaSamplerContext context) {
        ...
        result.setSampleLabel("AddLogByHash");
        result.sampleStart();
        ResponseData<Boolean> rr = evidenceService.addLogByHash (hash,"123",weIdAuthentication.getWeIdPrivateKey());
        if (rr.getResult() == null || rr.getResult()==false) {
            try {
                throw new Exception("AddLogByHash:" +
                        rr.getErrorCode() + ", " +
                        rr.getErrorMessage());
            } catch (Exception e) {
                e.printStackTrace();
                getNewLogger().error("------------->AddLogByHash fail:", e);
                result.sampleEnd();
            ...
            getNewLogger().info("----------------->AddLogByHash"+rr.toString());
            result.sampleEnd();
        }
        return result;
    }
