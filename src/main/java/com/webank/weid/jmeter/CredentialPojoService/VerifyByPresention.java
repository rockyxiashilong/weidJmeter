...
public class VerifyByPresention extends AbstractJavaSamplerClient {
    ...
    private static volatile PresentationE presentationE;
    ...
    static {
        ...
        if (presentationE == null){
            presentationE = credentialPojoService.createPresentation(
                    credentialList,
                    presentationPolicyE,
                    challenge,
                    weIdAuthentication).getResult();
        }
    }
    public static void main(String[] args) {
        ...
        VerifyByPresention test = new VerifyByPresention();
        test.setupTest(arg0);
        test.runTest(arg0);
    ...
    public void setupTest(JavaSamplerContext context) {
        getNewLogger().info("--------------------->this is VerifyByPresention setupTest");
...
//        arguments.addArgument("name", this.name);
    ...
    public SampleResult runTest(JavaSamplerContext context) {
        ...
        result.setSampleLabel("VerifyByPresention");
        result.sampleStart();
        ResponseData<Boolean>  rr = credentialPojoService.verify(
        ...
        if (rr.getResult() == null || rr.getResult()==false) {
            try {
                throw new Exception("VerifyByPresention:" +
                        rr.getErrorCode() + ", " +
                        rr.getErrorMessage());
            } catch (Exception e) {
                e.printStackTrace();
                getNewLogger().error("------------->VerifyByPresention fail:", e);
                result.sampleEnd();
            ...
            getNewLogger().info("----------------->VerifyByPresention"+rr.toString());
            result.sampleEnd();
        }
        return result;
    }
